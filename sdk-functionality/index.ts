...
// Group Chat Data 
const groupName = uniqueNamesGenerator({ dictionaries: [adjectives, colors, animals] }) 
...
// NFT Group Chat Data 
const nftGroupName = uniqueNamesGenerator({ dictionaries: [adjectives, colors, animals] }) 
const updatedNftGroupName = uniqueNamesGenerator({ dictionaries: [adjectives, colors, animals] }) 
...
`);
  ...
  console.log("PushAPI.payloads.sendNotification() [Direct Payload, Single Recipient]");
  await PushAPI_payloads_sendNotification__direct_payload_single_recipient();

  console.log("PushAPI.payloads.sendNotification() [Direct Payload, Batch of Recipients (Subset)]");
  await PushAPI_payloads_sendNotification__direct_payload_group_of_recipient_subset();

  console.log("PushAPI.payloads.sendNotification() [Direct Payload, All Recipients (Broadcast)]");
...
async function PushAPI_chat_updateGroup(chatId: string, silent: boolean = !showAPIResponse) {
  ...
  const response = await PushAPI.chat.updateGroup({
    ...
    groupName,
    groupDescription,
    members: [`eip155:${randomWallet1}`, `eip155:${randomWallet2}`, `eip155:${randomWallet3}`, `eip155:${signer.address}`],
    groupImage,
...
async function runNFTChatUseCases() {
  ...
  // Push Chat - PushAPI.user.create
  async function PushAPI_nft_user_create(silent: boolean = !showAPIResponse) {
    const user1 = await PushAPI.user.create({
      account: nftAccount1,
      signer: nftSigner1,
      env: env as ENV,
      additionalMeta: {NFTPGP_V1: {
        password: nftProfilePassword1 as string,
      }},
    })
    ...
    const user2 = await PushAPI.user.create({
      ...
      env: env as ENV,
      additionalMeta: {NFTPGP_V1: {
        password: nftProfilePassword2 as string,
      }},
    })
  ...
  // Push Chat - PushAPI.chat.decryptPGPKey
  async function PushAPI_nft_chat_decryptPGPKey(silent: boolean = !showAPIResponse) {
    // get user and derive encrypted PGP key
    const user = await PushAPI.user.get({
      account: nftAccount1,
      env: env as ENV
  ...
  // Push Chat - PushAPI.chat.chats
  async function PushAPI_nft_chat_chats(silent: boolean = !showAPIResponse) {
    // Fetch user
    const user = await PushAPI.user.get({
      account: nftAccount1,
      env: env as ENV,
  ...
  // Push Chat - PushAPI.chat.requests
  async function PushAPI_nft_chat_requests(silent: boolean = !showAPIResponse) {
    // Fetch user
    const user = await PushAPI.user.get({
      account: nftAccount1,
      env: env as ENV
  ...
  // Push Chat - PushAPI.chat.conversationHash
  async function PushAPI_nft_chat_conversationHash(silent: boolean = !showAPIResponse) {
    // conversation hash are also called link inside chat messages
    const conversationHash = await PushAPI.chat.conversationHash({
      account: nftAccount1,
  ...
  // Push Chat - PushAPI.chat.latest
  async function PushAPI_nft_chat_latest(silent: boolean = !showAPIResponse) {
    // Fetch user
    const user = await PushAPI.user.get({
      account: nftAccount1,
      env: env as ENV
  ...
  // Push Chat - PushAPI.chat.history
  async function PushAPI_nft_chat_history(silent: boolean = !showAPIResponse) {
    // Fetch user
    const user = await PushAPI.user.get({
      account: nftAccount1,
      env: env as ENV
  ...
  // Push Chat - PushAPI.chat.send
  // // Will send a message to the user or chat request in case user hasn't approved them
  async function PushAPI_nft_chat_send(silent: boolean = !showAPIResponse) {
    // Fetch user
    const user = await PushAPI.user.get({
      account: nftAccount1,
      env: env as ENV
  ...
  // Push Chat - Approve
  async function PushAPI_nft_chat_approve(silent: boolean = !showAPIResponse) {
    // Fetch user
    const user = await PushAPI.user.get({
      account: nftAccount2,
      env: env as ENV
  ...
  // Push Chat - PushAPI.chat.createGroup
  async function PushAPI_nft_chat_createGroup(silent: boolean = !showAPIResponse): Promise<string> {
    // Fetch user
    const user = await PushAPI.user.get({
      account: nftAccount1,
      env: env as ENV
  ...
  // Push Chat - PushAPI.chat.updateGroup
  async function PushAPI_nft_chat_updateGroup(chatId: string, silent: boolean = !showAPIResponse) {
    // Fetch user
    const user = await PushAPI.user.get({
      account: nftAccount1,
      env: env as ENV
  ...
  // Push Chat - PushAPI.chat.getGroupByName
  async function PushAPI_nft_chat_getGroupByName(silent: boolean = !showAPIResponse) {
    const response = await PushAPI.chat.getGroupByName({
      groupName: 'Push Group Chat 3',
      env: env as ENV
    })
  ...
  // Push Chat - PushAPI.chat.getGroup
  async function PushAPI_nft_chat_getGroup(silent: boolean = !showAPIResponse) {
    const response = await PushAPI.chat.getGroup({
      chatId: '870cbb20f0b116d5e461a154dc723dc1485976e97f61a673259698aa7f48371c',
      env: env as ENV
    })
  ...
  // Push Chat - PushAPI.chat.decryptConversation
  async function PushAPI_nft_chat_decryptConversation(silent: boolean = !showAPIResponse) {
    // Fetch user
    const user = await PushAPI.user.get({
      account: nftAccount1,
      env: env as ENV
